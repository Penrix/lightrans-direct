...
划词翻译沿用 compact 版本的小气泡，只显示译文，不恢复旧版“原文 + 译文”大面板。整页翻译继续遵守“仅原文 / 仅译文 / 对照”设置。

翻译路径现在优先使用明确语言对：如果源语言设置为“自动”，扩展会先在本地按文本中的主要文字脚本判断英语 / 中文；判断明确后再把例如 `en → zh-CN` 这样的语言对交给翻译模型。整页翻译也会尊重设置里的源语言，不再无条件把 `auto` 传给模型。

普通英文单词不需要靠术语补丁强制翻译。固定术语表只保留为用户主动控制最终译法或保留原词的覆盖层，每行格式：
...
第一条固定译法，第二条明确保留原词。未出现在术语表里的英文按正常英译中处理。固定术语使用占位符保护，在模型翻译之后确定性恢复，不依赖模型是否记住提示词。
