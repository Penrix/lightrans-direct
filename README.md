...
默认翻译路径现在优先使用明确语言对：如果源语言设置为“自动”，扩展会先在本地按文本中的主要文字脚本判断英语 / 中文；判断明确后再把例如 `en → zh-CN` 这样的语言对交给翻译模型。整页翻译也会尊重设置里的源语言，不再无条件把 `auto` 传给模型。

因此普通英文单词不需要靠术语补丁强制翻译。固定术语表只保留为用户主动控制最终译法或保留原词的覆盖层，每行格式：
